...
- **Request**: `amount` (양수)
- **Response (200)**: 성공 메시지
- **정책**: 일 한도 1,000,000원 (UTC 기준)
- **Error**: 400 (검증 실패, 잔액 부족, 한도 초과, 계좌 폐쇄), 404 (계좌 없음)

### 이체
- **POST** `/transfers`
- **Request**: `fromAccountNumber`, `toAccountNumber`, `amount` (양수, 최대 1억원)
- **Response (200)**: 성공 메시지
- **정책**: 수수료 1%, 일 한도 3,000,000원 (UTC 기준)
- **Error**: 400 (검증 실패, 잔액 부족, 한도 초과, 계좌 폐쇄), 404 (계좌 없음)
...
### 보낸 이체 내역
- **GET** `/accounts/{accountId}/sent-transfers`
- **Response**: `transactionId`, `amount`, `fee`, `counterpartyAccountNumber`, `createdAt`

### 받은 이체 내역
- **GET** `/accounts/{accountId}/received-transfers`
